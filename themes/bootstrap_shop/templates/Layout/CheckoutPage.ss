	...
	<div class="row">
		<div class="col-lg-12">
			...
			<div id="Checkout" class="accordion">
				...
				<div class="panel panel-default">
					<div class="panel-heading">
						<% if IsPastStep(discount) %>
							<h3 class="panel-title"><a title="choose payment method" href="$Link(discount)">Discount <i class="icon icon-pencil editicon"></i><i class="icon icon-lock pull-right"></i></a></h3>
						<% else %>
							<h3 class="panel-title">Discount <i class="icon icon-lock pull-right"></i></h3>
						<% end_if %>
					</div>
					<% if IsFutureStep(discount) %>

					<% else %>
						<div class="panel-body">
							<% if IsCurrentStep(discount) %>
								$CouponForm
							<% end_if %>
							<% if IsPastStep(discount) %>
								$CouponForm
